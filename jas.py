...
class BasePasswordManager:
    #Holding old passwords in a list
    old_passwords = list()
    #returns current password as a string
    def get_password(self):
        try:
            return self.old_passwords[-1]
        except:
            print("Empty! No Old Passwords available.")
            exit()
# receives a string and returns a boolean
...
# Inheritance of BasePasswordManager
class passwordmanager(BasePasswordManager):
    ...
    def set_password(self,new_password):
        if len(new_password)<6:
            print("Sorry!, minimum 6 characters is must for password.")
            exit()
        else:
...
print()
print("Enter Another Password For Demonstration!...")
